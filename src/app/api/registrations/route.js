...
export async function GET(req) {
  try {
    ...
    const conf = searchParams.get('conference') || 'cyber';
...
export async function POST(req) {
  try {
    ...
    const { name, email, category, conference = 'cyber' } = body;
    const account = CONFERENCE_ACCOUNTS.find(acc => acc.conferenceId === conference);
    const adminEmail = account ? account.email : 'liutex@sciengasummits.com';
